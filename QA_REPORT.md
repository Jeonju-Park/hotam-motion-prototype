...
## 4-F. 최종 검토 패스 (08-19 · Fable)

### 더미 사진
`z_image` 모델로 12장 생성(식당 10 + 피드 보조 2) -> 640px JPEG로 압축해 `assets/food/f1~f12.jpg` **로컬 번들**.
외부 이미지 URL 금지(스펙 §1.3)는 로컬 파일이므로 저촉 없음. 적용 위치: 피드 스트립 · 상세 히어로 ·
탐색 시트 썸네일 · 온보딩(선택 행·평가 카드) · 기록 모달(검색 행·수준 카드·비교 카드) · 프로필 3열 그리드 ·
랭킹 식당 행 · 검색 결과 행. `img` 없는 항목은 기존 톤+아이콘 폴백 유지(`thumb()` 헬퍼).

### 모션·인터랙션 전수 점검에서 추가로 잡은 것

| # | 항목 | 수정 |
|---|---|---|
| 1 | 인풋 focus/error 보더 전환이 `enter` 곡선 | 가이드 §3 명세대로 **d3·standard** + border-width 전환 추가 |
| 2 | 다이얼로그 scrim 소멸 지연 60ms | 가이드 §3 명시값 **50ms(d1)** 로 교정 |
| 3 | 비교 스텝이 선택 시 **전체 리렌더**(두 카드 모두 재진입) | 스펙 §6 REC ③ "선택 시 반대편 교체"대로 — 내 카드는 유지, **도전자 카드만** 아래로 퇴장(d3·exit) 후 재진입(d5·emphasized-enter). 연타 가드 포함 |
| 4 | 검색 식당/미식가 세그 전환이 즉시 교체 | Trans3 순차 페이드 적용 |
| 5 | 피드 장소 바가 보더형 | 피그마대로 **면 채움(bg-surface)** · 사진 라운드 r-lg |
| 6 | 정적 에셋 캐시 스테일(배포 후 옛 JS가 남는 문제) | `?v=` 버전 쿼리를 4개 에셋에 부착 |

### 전수 점검표 (가이드 §2~§5 × 코드 대조)
Push/Pop/Modal/TabFade/Root 파라미터 · 입력 잠금 300ms · 탭 스크롤 보존 · 재탭 최상단 ·
버튼 press/release·loading · 칩 선택 d3/해제 d2 · 찜 낙관 토글 · 토스트 d4/d2·교체 ·
다이얼로그 d5/d3·scrim 50ms · 시트 d6/순차 120·160·200/퇴장 d4 · 배너 · 스켈레톤 200/300/pulse 1200 ·
인풋 d3·standard·shake · 점수 공개 타임라인·스킵·이전 점수 · 스냅 3단·저항 0.3·속도 0.5 ·
그러버 32x4 · Full 검색바 d2 · dismiss 30% · scrim 연동 · 미리보기 40%/복원/하향 dismiss ·
햅틱 3종·병합 · reduced-motion 표 전체 — **전 항목 코드 확인 완료**. 이탈은 위 표 6건 + 기존 결정 로그(Pop 통일)뿐.

기록 플로우 종단 재검증: 수준 선택 -> 비교(도전자 교체: 라멘연구소 -> 소금빵집) -> 점수 공개(9.6, 타임라인 5단계) -> 홈 최상단 내 기록 삽입. 콘솔 에러 0.
