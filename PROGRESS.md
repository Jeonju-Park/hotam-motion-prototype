...
## 4차 — 최종 검토 (08-19 · Fable)

| 시각 | 내용 | 검증 |
|---|---|---|
| 19:40 | 더미 사진 12장 생성(z_image) -> assets/food 로컬 번들, 9개 화면 영역 연결(`thumb()` 폴백 포함) | 스크린샷 |
| 19:50 | 모션 전수 점검: 인풋 d3·standard · 다이얼로그 scrim 50ms · 비교 "반대편 교체" · 검색 세그 페이드 · 장소 바 면 채움 | QA_REPORT §4-F |
| 20:00 | 에셋 `?v=` 버전 쿼리(캐시 스테일 방지) · 기록 플로우 종단 재검증(교체->9.6->홈 삽입) | 콘솔 0 |
| 20:05 | verify 12 pass / 0 fail · 스크린샷 18장 · 배포 | Pages |

## 5차 — 정주 피드백 6건 (08-19)

| 시각 | 내용 | 검증 |
|---|---|---|
| 20:30 | (1) 푸시/팝 z-order 관리(z-top) — DOM 역순 쌍(검색->상세 등)에서 이전 화면 복귀만 보이던 문제 해결 | 브라우저 재현 |
| 20:35 | (2) 시트 Full = 95vh 위치(738px) — 탭바 오프셋 포함 시 100%처럼 꽉 차던 것 교정 | snaps [200,422,738] |
| 20:40 | (3) 기록 모달 = 풀페이지 개념: top 0 + 상태바를 모달 위 레이어(z 66)로, 점수 공개 딤에서만 흰색 | 스크린샷 |
| 20:45 | (4) 스켈레톤을 실제 피드/리스트 실루엣으로 재작성(sk-feed·sk-feed--row) | 05_home_loading |
| 20:50 | (5) 상세 CTA -> Trans2 확인(vpBehind·풀페이지) · (6) backdrop-filter 3곳 제거, 전역 .body 전환 스코프 축소 | 브라우저 재현 |
| 21:10 | 전환 엔진 프레임-세이프화: 시작 상태를 한 프레임 먼저 페인트 후 목표 상태 전환(push/pop/tabfade/root/modal). 엔진별 전환 스킵 방지 | 온보딩 push 390->3 수치 회귀 |
